...
impl Agent {
    ...
    fn attach_workflow_verification(
        ...
        if !evaluation.verification.satisfied {
            let issue = format!("Workflow incomplete: {}", evaluation.verification.summary);
            if !task_result.unresolved_issues().contains(&issue) {
                task_result = task_result.with_unresolved_issue(issue);
            }
        }

        task_result.with_workflow_verification(evaluation.verification)
    }
