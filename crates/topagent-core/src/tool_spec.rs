...
impl ToolSpec {
    ...
    pub fn bash() -> Self {
        Self {
            name: "bash".to_string(),
            description: "execute bash command in workspace directory (commands run in a workspace sandbox; network is controlled by the active access profile; output truncated at 64KB per stream)".to_string(),
            input_schema: serde_json::json!({
                "type": "object",
                "properties": {
